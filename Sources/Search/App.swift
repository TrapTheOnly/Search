...
struct ContentView: View {
    ...
    private func watchKeys() {
        ...
        keys = NSEvent.addLocalMonitorForEvents(matching: [.keyDown, .flagsChanged]) { event in
            guard event.type == .keyDown else {
                ...
                if browser.tabSwitcher.active, !event.modifierFlags.contains(.option) {
    ...
    private func take(_ event: NSEvent) -> Bool {
        ...
        // ⌥Tab: MRU tabs in this space. A quick tap jumps to the most recent
        // other tab; holding ⌥ past a short delay shows the sneak-peek
        // overlay. ⌃Tab still walks the strip in order (below).
        if event.keyCode == 48, flags.contains(.option),
        ...
        if browser.tabSwitcher.active, flags.contains(.option),
           !flags.contains(.command), !flags.contains(.control) {
    ...
    private var canSwitchTabs: Bool {
        ...
        return !browser.tuning && !browser.recalling && !browser.hoarding &&
            ...
            !browser.managingProfiles && !browser.reviewing && !browser.finding &&
