...
/// One Option+Tab gesture. Candidate order stays fixed until Option is
/// released, so stepping does not rearrange the list mid-walk. A short hold
/// delay keeps a quick Option+Tab as a silent jump to the most recent other
/// tab; holding past the delay shows the sneak-peek overlay.
@MainActor
final class TabSwitcher: ObservableObject {
    enum Direction { case left, right, up, down }

    /// Delay before the overlay appears — quick Option+Tab skips it.
...
/// Windows-style Alt+Tab sneak-peek: tab previews in the current space,
/// most recently used first. Lives above the page and address field.
    ...
    var body: some View {
        if switcher.visible {
                            ...
                            ForEach(0..<((switcher.candidates.count + 4) / 5), id: \.self) { row in
                                HStack(spacing: 8) {
                                    ForEach(Array(switcher.candidates.dropFirst(row * 5).prefix(5)), id: \.self) { id in
                                        if let tab = browser.strip.first(where: { $0.id == id }) {
            ...
            .task(id: switcher.selectedID) {
                ...
                switcher.capturePreviews(from: browser.strip, current: browser.activeID)
            }
        }
    }
