...
final class Browser: NSObject, ObservableObject {
    ...
    func select(_ tab: Tab) {
        ...
        // Gate on the raw membership ids — not `splitPanes`, which is nil when
        // `find` fails even though left/right are still set, and would fall
        // through to remapping instead of clearing. Member selects keep the
        // pair and only remapping the mate pointer.
        leaveSplitUnlessMember(tab)
        ...
        leaving()
        activeID = tab.id
        // Mate tracks the non-focused pane after focus moves (pane-to-pane).
        if isSplitActive { syncSplitMate() }
        tab.touch()
